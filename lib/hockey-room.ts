...
type Stored = { roster:Player[]; frame:Frame|null; inputs:Record<string,Input>; lastTick:number };
...
const PUCK_R=.024, MALLET_R=.052, LEFT=.03, RIGHT=.97, TOP=.045, BOTTOM=.955, GOAL_MIN=.36, GOAL_MAX=.64, MAX_PUCK=.82, MAX_MALLET=1.25, TARGET=7;
...
function decode(raw:unknown,row:Row):Stored{
  ...
  if(raw&&typeof raw==="object"&&!Array.isArray(raw)){
    ...
    if(Array.isArray(v.roster))return{roster:v.roster as Player[],frame:v.frame&&typeof v.frame==="object"?v.frame as Frame:null,inputs:v.inputs&&typeof v.inputs==="object"?v.inputs as Record<string,Input>:{},lastTick:finite(v.lastTick,now)};
  }
  if(Array.isArray(raw)){
    const roster=raw as Player[];
    return{roster,frame:row.status==="playing"||row.status==="gameover"?fresh(roster,row.mode,now,row.left_score,row.right_score,row.winner_side):null,inputs:{},lastTick:now};
  }
  return{roster:[],frame:null,inputs:{},lastTick:now};
}
function output(row:Row,s:Stored){return{mode:row.mode,status:row.status,players:s.roster,leftScore:s.frame?.leftScore??Number(row.left_score||0),rightScore:s.frame?.rightScore??Number(row.right_score||0),winnerSide:s.frame?.winnerSide??row.winner_side,targetScore:TARGET,frame:s.frame}}
